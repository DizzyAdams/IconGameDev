# IconHub — Roblox UGC Experience Kit
...
- `ServerScriptService/SurrealCosmetics.server.luau` — server-authoritative "dreamscape" cosmetic applier: turns owned GamePasses into original visual effects (trails, auras, wings, halos, pets, capes, banners). Reads the `GamePass_<id>` attributes set by GamePassService; client never decides.
...
2. Open this folder in a terminal and run `rojo build -o IconHub.rbxlx` then open the file, or `rojo serve` and connect from the plugin.
