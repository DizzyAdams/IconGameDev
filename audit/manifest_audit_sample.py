...
def main():
    ...
    if len(manifest_files) > SAMPLE_SIZE:
        manifest_files = manifest_files[:SAMPLE_SIZE]
    print(f"Checking {len(manifest_files)} manifest files (sample of {SAMPLE_SIZE})")
    ...
    if all_issues:
        print("\n--- ISSUES ---")
        for item in all_issues[:10]:  # limit output
            print(f"{item['file']}:")
            if "error" in item:
    ...
    # Write report
    report_path = Path("audit/manifest_report_sample.json")
    report_path.parent.mkdir(exist_ok=True)
    report = {
        "total_checked": len(manifest_files),
        "issues": all_issues,
